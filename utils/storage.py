...
DB_PATH = "astro.db"
DDL = '''
CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, created_at TEXT);
CREATE TABLE IF NOT EXISTS usage (user_id INTEGER PRIMARY KEY, window_start TEXT, count INTEGER);
CREATE TABLE IF NOT EXISTS logs (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, command TEXT, payload TEXT, created_at TEXT);
'''
def init_db():
...
def get_usage(user_id: int):
    conn = sqlite3.connect(DB_PATH); cur = conn.cursor()
    cur.execute("SELECT window_start, count FROM usage WHERE user_id=?", (user_id,)); row = cur.fetchone()
