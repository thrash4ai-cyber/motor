...
def usage_ok(user_id: int) -> bool:
    ...
    now = datetime.utcnow()
    if not row:
        set_usage(user_id, now.isoformat(), 0)
        return True
    window_start_iso, count = row
    try:
        fromiso = datetime.fromisoformat(window_start_iso)
    except Exception:
        fromiso = now
    if now - fromiso >= timedelta(hours=24):
        set_usage(user_id, now.isoformat(), 0)
        return True
...
def menu_keyboard():
    return ReplyKeyboardMarkup([
        [KeyboardButton("/energy"), KeyboardButton("/compat")],
        [KeyboardButton("/ask"), KeyboardButton("/help")]
...
async def energy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not usage_ok(update.effective_user.id):
        await update.message.reply_text(texts.LIMIT_HIT)
        return ConversationHandler.END
    await update.message.reply_text(texts.ASK_DATE)
    return ASK_BIRTH
...
async def energy_calc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    e = energy_of_day(d, today)
    sign = zodiac_sign(d)
    bump_usage(user_id)
    log_event(user_id, "energy", update.message.text)
    text = (
        f"Путь жизни: {e['life_path']} | День: {e['day_num']} | Сумма: {e['mix']}\n"
        f"Знак: {sign}\n"
        f"Фокус: {e['meaning']}\n\n"
        f"(Полный разбор с дополнительными инсайтами — в Plus.)\n{texts.CTA}"
    )
...
async def compat(update: Update, Context: ContextTypes.DEFAULT_TYPE):
    if not usage_ok(update.effective_user.id):
        await update.message.reply_text(texts.LIMIT_HIT)
        return ConversationHandler.END
    await update.message.reply_text(texts.ASK_TWO_DATES)
    return ASK_COMPAT
...
async def compat_calc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not d1 or not d2:
        await update.message.reply_text(texts.INVALID_TWO_DATES)
        return ASK_COMPAT
    sc = compatibility_score(d1, d2)
    bump_usage(user_id)
    log_event(user_id, "compat", update.message.text)
    text = (
        f"Пути жизни: {sc['lp1']} & {sc['lp2']}\n"
        f"Быстрая гармония: {sc['score']}/100\n"
        f"Риск: {sc['risk']}\n"
        f"Совет: {sc['tip']}\n\n"
        f"(Глубокая совместимость с 5–7 инсайтами — в Plus.)\n{texts.CTA}"
    )
    await update.message.reply_text(text, reply_markup=menu_keyboard())
    return ConversationHandler.END
...
async def ask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not usage_ok(update.effective_user.id):
        await update.message.reply_text(texts.LIMIT_HIT)
        return ConversationHandler.END
    await update.message.reply_text(texts.ASK_QUESTION)
    return ASK_QUESTION

def _sanitize_question(q: str) -> str:
    return (q or "").strip()[:200]
...
async def ask_calc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"Твой вопрос: «{q}»\n\n" + "\n".join(bullets) + "\n\n" +            "(Более точный тайминг и разговорные коды — в Plus.)\n" + texts.CTA
    await update.message.reply_text(text, reply_markup=menu_keyboard())
    return ConversationHandler.END
...
def build_app():
    ...
    energy_conv = ConversationHandler(
        ...
        states={
            ASK_BIRTH: [MessageHandler(filters.TEXT & ~filters.COMMAND, energy_calc)]
        },
        fallbacks=[CommandHandler("start", start)]
    )
    compat_conv = ConversationHandler(
        entry_points=[CommandHandler("compat", compat)],
        states={
            ASK_COMPAT: [MessageHandler(filters.TEXT & ~filters.COMMAND, compat_calc)]
        },
        fallbacks=[CommandHandler("start", start)]
    )
    ask_conv = ConversationHandler(
        ...
        states={
            ASK_QUESTION: [MessageHandler(filters.TEXT & ~filters.COMMAND, ask_calc)]
        },
        fallbacks=[CommandHandler("start", start)]
    )
...
def main():
    ...
    if MODE == "polling":
        ...
        if not WEBHOOK_BASE:
            raise RuntimeError("WEBHOOK_BASE is not set but MODE=webhook")
        url_path = f"/webhook/{TOKEN}"
        webhook_url = WEBHOOK_BASE + url_path
